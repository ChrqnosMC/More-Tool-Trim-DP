...
async function writeItemRecipes() {
  ...
  await mkdir(`./generator/output/data/more_tool_trim/recipe`, { recursive: true })
    ...
    for (const trim of trims) {
      for (const [materialIndex, [material]] of Object.entries(items).entries()) {
        ...
        const recipeFile = isSameMaterial
          ? `./generator/output/data/more_tool_trim/recipe/${tool}_smithing_from_${trim}_and_${material}_darker.json`
          : `./generator/output/data/more_tool_trim/recipe/${tool}_smithing_from_${trim}_and_${material}.json`
        const recipeContents = {
          ...
          result: {
            ...
            components: {
              ...
              'minecraft:trim': {
                material: materialIndex < 10 ? `minecraft:${material}` : `more_tool_trim:${material}`,
